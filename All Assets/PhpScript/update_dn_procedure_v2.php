...
echo "=== MISE À JOUR DE LA PROCÉDURE GetDnDetails (v2) ===\n\n";
...
'COALESCE(tp_related.Label, "ETABLISSEMENT MULTI SERVICES NAPAONGO") AS secondaryClient, ' .
...
'DATE_FORMAT(COALESCE(DATE_ADD((SELECT inv.ValIdationDate FROM invoice inv JOIN invoiceitem ii ON inv.Id = ii.InvoiceId WHERE ii.JobFileId = jf.Id LIMIT 1), INTERVAL 5 DAY), DATE_ADD(NOW(), INTERVAL 7 DAY)), "%d/%m/%Y") AS badValidityDate, ' .
'CONCAT("<strong>ATL Nr:</strong> 1064697/1250223/138<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4025") AS atlNumber, ' .
'DATE_FORMAT(COALESCE(DATE_ADD((SELECT inv.ValIdationDate FROM invoice inv JOIN invoiceitem ii ON inv.Id = ii.InvoiceId WHERE ii.JobFileId = jf.Id LIMIT 1), INTERVAL 5 DAY), DATE_ADD(NOW(), INTERVAL 7 DAY)), "%d/%m/%Y") AS badValidUntil, ' .
'JSON_OBJECT(' .
...
'GROUP BY jf.Id, tp.Label, tp_related.Label, bl.BlNumber, c.CallNumber ' .
'LIMIT 1; ' .
'END';
...
if (mysqli_query($conn, $proc)) {
    ...
    if ($result) {
        ...
        if ($row) {
            ...
            echo "  - Invoice Validity Date: " . $row['invoiceValidityDate'] . "\n";
            echo "  - BAD Validity Date: " . $row['badValidityDate'] . "\n";
            echo "  - BAD Valid Until: " . $row['badValidUntil'] . "\n";
