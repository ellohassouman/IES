...
'COMMENT "Récupérer les détails complets de la facture" BEGIN SELECT ' .
'inv.Id AS invoiceId, ' .
...
'"NZ083876" AS account, ' .
'COALESCE(tp_related.Label, "SOCIETE IVOIRIENNE DE PARFUMERIE") AS secondaryClient, ' .
'DATE_FORMAT(COALESCE(c.VesselArrivalDate, NOW()), "%d/%m/%Y") AS shipArrivalDate, ' .
'DATE_FORMAT(COALESCE(c.VesselDepatureDate, NOW()), "%d/%m/%Y") AS shipDepartureDate, ' .
...
'"euroValue", "192.85 EUR") AS totals, ' .
'COALESCE((SELECT GROUP_CONCAT(CONCAT(bli.Number, "(", COALESCE(bit.Label, ""), ")") SEPARATOR ", ") ' .
'FROM invoiceitem ii ' .
...
'WHERE ii.InvoiceId = inv.Id LIMIT 1), "TLLU2088717(22G1, CAT 3, E3 - Autres produits non recensés)") AS containers, ' .
'1 AS success ' .
'FROM invoice inv ' .
...
'LEFT JOIN thirdparty tp_related ON bl.RelatedCustomerId = tp_related.Id ' .
'LEFT JOIN ' . $backquote . 'call' . $backquote . ' c ON bl.CallId = c.Id ' .
'WHERE inv.Id = p_InvoiceId AND inv.Deleted = 0 ' .
'GROUP BY inv.Id, inv.InvoiceNumber, tp.Label, tp_related.Label, inv.BillingDate, inv.ValIdationDate, bl.BlNumber, c.CallNumber, c.VesselArrivalDate, c.VesselDepatureDate ' .
'LIMIT 1; ' .
'END';
